...
#[cfg(feature = "avx")]
fn bench_fft_avx(b: &mut Bencher, len: usize) {
...
#[cfg(feature = "avx")]
fn bench_realfft_avx(b: &mut Bencher, len: usize) {
    let mut planner = RealFftPlannerAvx::<f64>::new().unwrap();
    let fft = planner.plan_fft_forward(len);
...
#[cfg(feature = "avx")]
fn bench_ifft_avx(b: &mut Bencher, len: usize) {
    let mut planner = rustfft::FftPlannerAvx::new().unwrap();
    let fft = planner.plan_fft_inverse(len);
    let mut scratch = vec![Complex::from(0.0); fft.get_outofplace_scratch_len()];
...
#[cfg(feature = "avx")]
fn bench_realifft_avx(b: &mut Bencher, len: usize) {
    let mut planner = RealFftPlannerAvx::<f64>::new().unwrap();
    let fft = planner.plan_fft_inverse(len);
...
fn bench_pow2_fw(c: &mut Criterion) {
    ...
    for i in [8, 16, 32, 64, 128, 256, 1024, 4096, 65536].iter() {
        ...
        #[cfg(feature = "avx")]
        group.bench_with_input(BenchmarkId::new("Complex AVX", i), i, |b, i| {
            bench_fft_avx(b, *i)
        });
        #[cfg(feature = "avx")]
        group.bench_with_input(BenchmarkId::new("Real AVX", i), i, |b, i| {
            bench_realfft_avx(b, *i)
        });
    }
    group.finish();
}
